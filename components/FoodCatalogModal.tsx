...
import { Search, X, Check, Utensils, Info, ChevronRight } from 'lucide-react';
...
export const FoodCatalogModal: React.FC<FoodCatalogModalProps> = ({ mealType, onClose, onAdd }) => {
  ...
  // Refinement states - initialized with Standard defaults
  ...
  const filtered = useMemo(() => {
    ...
    if (!s) return FOOD_CATALOG.filter(f => f.category === mealType).slice(0, 40);
  ...
  const calculatedKcal = useMemo(() => {
    ...
    // Base Math
    let base = (refineItem.kcalPer100g / 100) * grams;
  ...
  const commitAdd = () => {
    ...
    const tags = [];
    if (fatLevel !== 'Standard') tags.push(fatLevel);
    if (sauceLevel !== 'None') tags.push(`${sauceLevel} Sauce`);
    if (origin !== 'Standard') tags.push(origin);
    if (extras !== 'None') tags.push(`+ ${extras}`);

    onAdd({
      id: Math.random().toString(36).substr(2, 9),
      foodId: refineItem.id,
      name: `${refineItem.name}${tags.length ? ` (${tags.join(', ')})` : ''}`,
      kcal: calculatedKcal,
      grams: grams,
      mealType,
      timestamp: Date.now(),
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/80 backdrop-blur-sm transition-all">
      <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-[40px] sm:rounded-[48px] shadow-2xl flex flex-col max-h-[95vh] overflow-hidden border border-white/5">
          ...
          {!refineItem ? (
            <>
              <div className="relative group">
                ...
                <input autoFocus className="w-full bg-slate-100 dark:bg-slate-800 rounded-3xl py-5 pl-16 pr-8 text-sm font-black text-slate-700 dark:text-slate-200 border-none focus:ring-4 focus:ring-indigo-500/10 transition-all" placeholder="Search dish name..." value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
            ...
            <div className="space-y-10 animate-in slide-in-from-bottom-6">
              {/* Summary Card */}
              <div className="p-8 bg-indigo-600 rounded-[44px] shadow-2xl text-white border border-indigo-500 flex justify-between items-center">
                <div className="space-y-1">
                  <h3 className="font-black text-lg leading-tight">{refineItem.name}</h3>
                  <div className="flex items-center gap-2 opacity-70"><Info size={12}/> <span className="text-[9px] font-black uppercase tracking-widest">Estimated Energy</span></div>
                </div>
                <div className="text-5xl font-black text-right">{calculatedKcal} <div className="text-[10px] opacity-60 uppercase tracking-widest">kcal</div></div>
              </div>

              {/* Refinement Questions */}
              <div className="space-y-10">
                <div className="space-y-5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">1. Grams Consumed</label>
                  <div className="grid grid-cols-4 gap-2">
                    {[50, 100, 200, 300].map(v => (
                      <button key={v} onClick={() => setGrams(v)} className={`py-4 rounded-2xl text-[10px] font-black uppercase transition-all border-2 ${grams === v ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-400'}`}>
                        {v}g
                      </button>
                    ))}
                  </div>
                  <input type="number" className="w-full bg-slate-50 dark:bg-slate-800 rounded-2xl py-4 px-6 text-sm font-black border-none" value={grams || ''} onChange={(e) => setGrams(Number(e.target.value))} placeholder="Manual grams..." />
                </div>

                <div className="space-y-5">
                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">2. Cooking Oil Level</label>
                   <div className="grid grid-cols-3 gap-2">
                     {(['Light', 'Standard', 'Oily'] as const).map(f => (
                       <button key={f} onClick={() => setFatLevel(f)} className={`py-4 rounded-2xl text-[10px] font-black uppercase transition-all border-2 ${fatLevel === f ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-400'}`}>{f}</button>
                     ))}
                   </div>
                </div>

                <div className="space-y-5">
                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">3. Sauce / Cream</label>
                   <div className="grid grid-cols-3 gap-2">
                     {(['None', 'Light', 'Heavy'] as const).map(s => (
                       <button key={s} onClick={() => setSauceLevel(s)} className={`py-4 rounded-2xl text-[10px] font-black uppercase transition-all border-2 ${sauceLevel === s ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-400'}`}>{s}</button>
                     ))}
                   </div>
                </div>
                ...
                <div className="space-y-5">
                   ...
                   <div className="grid grid-cols-3 gap-2">
                     {(['Home-made', 'Standard', 'Restaurant'] as const).map(o => (
                       <button key={o} onClick={() => setOrigin(o)} className={`py-4 rounded-2xl text-[10px] font-black uppercase transition-all border-2 ${origin === o ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-400'}`}>{o}</button>
                     ))}
                   </div>
                </div>
                ...
                <div className="space-y-5">
                   ...
                   <div className="grid grid-cols-4 gap-2">
                     {(['None', 'Bread', 'Soda', 'Both'] as const).map(ex => (
                       <button key={ex} onClick={() => setExtras(ex)} className={`py-4 rounded-2xl text-[10px] font-black uppercase transition-all border-2 ${extras === ex ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-400'}`}>{ex}</button>
                     ))}
                   </div>
                </div>

                <div className="space-y-5">
                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">6. Toppings / Cheese</label>
                   <div className="grid grid-cols-4 gap-2">
                     {(['None', 'Cheese', 'Nuts', 'Sauce'] as const).map(t => (
                       <button key={t} onClick={() => setToppings(t)} className={`py-4 rounded-2xl text-[10px] font-black uppercase transition-all border-2 ${toppings === t ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-400'}`}>{t}</button>
                     ))}
                   </div>
                </div>
              </div>
              ...
              {/* Final Actions */}
              <div className="sticky bottom-0 pt-4 pb-8 bg-white dark:bg-slate-900 flex gap-4">
